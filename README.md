<div align="center"><a href="https://www.escuelaing.edu.co/es/investigacion-e-innovacion/centro-de-estudios-hidraulicos/" target="_blank"><img src="https://github.com/rcfdtools/R.TeachingResearchGuide/blob/main/CaseUse/.icons/IconCEHBanner.jpg" alt="R.LTWB" width="100%" border="0" /></a></div>

# C.RCI - SISTEMAS DE RED CONTRA INCENDIO 

<div align="center">
<b> Universidad Escuela Colombiana de Ingeniería Julio Garavito</b>
<br> Andrea Liseth Vasco Chivatá 
<br>  Profesor del Centro de Estudios Hidráulicos 
<br>  andrea.vasco@escuelaing.edu.co 
</div>

<br>



<div align="center">
    <a href="https://youtu.be/Zv-WXSdbSyY">
        <img src="https://github.com/Andrealvch/C.RCI/blob/main/.graph/INTRODUCCION_CURSO.PNG" width="800px">
    </a>
</div>

<br>

# Introducción

Bienvenidos al curso virtual básico de sistemas de redes contra incendio ofrecido por la Universidad Escuela Colombiana de Ingeniería Julio Garavito. En el desarrollo del curso encontrarás videos que orientan el trabajo de cada uno de los módulos y sus respectivas actividades. En cada actividad encontrarás la información necesaria para cumplir con cada uno de los objetivos propuestos. 

Este curso corto está diseñado para que el estudiante adquiera los conceptos básicos sobre las redes contra incendio y sus respectivos complementos para cumplir con su objetivo específico de salvaguardar las vidas humanas, generar el menor impacto en la infraestructura de un edificio y sus activos. También se describirán los elementos que componen un sistema red contra incendio, los materiales utilizados en su contrucción y un ejemplo práctico del diseño de una red contra incendio con base en las normas Colombianas (NRS-10). 

Si bien es cierto que la Norma Colombiana de Diseño y Construcción Sismo Resistente NSR-10 establece los requisitos y procedimientos para el diseño y construcción de estructuras, incluyendo la protección de la edificación contra incendios, durante el desarrollo del curso, el estudiante podrá identificar que esta norma direcciona para el diseño de los sistemas de red contra incendio a la National Fire Protection Association (NFPA), organización que crea y mantiene las normas para la prevención contra incendio. Esta norma establece 8 tipos diferentes de sistemas de extinción y control de fuego que incluyem entre otros los agentes limpios, aerosoles, nebulización, etc. Para cada uno de estos tipos de sistemas se explica cada norma y así como diferentes características a tener en cuenta. 

Recuerda que puedes realizar el curso sin o con acompañamiento del profesor en cuyo caso podrá obtener el certificado. 

# Objetivos 
El objetivo general de este curso es familiarizar al estudiante con los sistemas contra incendio y la necesidad de su implementación en las edificaciones. El objetivo particular es presentar de manera adecuada, los conceptos incluídos en las normas NFPA (Americanas) teniendo en cuenta que son normas utilizadas en Colombia de acuerdo con lo recomendado por la NSR 10. Las normas NFPA plantean diferentes sistemas de red contra incendio de acuerdo con el tipo de edificación. 

- Familiarizarse con la terminología utilizada mundialmente por la organización fundada en Estados Unidos, la Asociación Nacional de Protección Contra el Fuego.
- Crear herramientas para facilitar la compresión de las normas NFPA para los sistemas de redes contra incendio.
- Aplicar la metodología de diseño establecido por la NFPA para los sostemas de redes contra incendio.
- Análisis de los resultados obtenidos durante el ejemplo. 

# Contenido 

## Sección 1 - conceptos generales y complementos

En esta sección se presentan las definiciones principales necesarias para el curso, tipos de redes y sistemas complementarios para la red contra incendio como extintores, señalización y automatización. 

| Actividad                                                                                                                          | Alcance                                                                                                                                                                                                                                                 |
|:-----------------------------------------------------------------------------------------------------------------------------------|:--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| [Bienvenida, introducción general y objetivos](Section01/Readme.md)                                                                                       | En esta clase se exponen los aspectos generales del curso y se definen sus objetivos generales.                                                                                                                                                          |
| [Definiciones](Section01/Definiciones)                                               | Para comprender fácilmente la terminología utilizada en este curso se definen algunos términos útiles en el lenguaje de las redes contra incendio.                                                                                                                                                     |
| [Normas utilizadas para el diseño de los sistemas de red contra incendio](Section01/Normas)                                                        | En esta actividad se describen las normas colombianas, utilizadas para el diseño de los sistemas de red contra incendio en una edificación.                                                                                                                        |
| [Extintores (NFPA 10)](Section01/Extintores)                                                                                                               | Se aborda la Norma para Extintores Portátiles Contra Incendio - NFPA 10, indicando las estipulaciones previstas para la selección, instalación, inspección, mantenimiento y pruebas de estos equipos.                                                   |
| [Señalización (ISO 7010)](Section01/Señalización)                                                                                                            | La norma técnica ISO 7010 define los símbolos gráficos sobre las señales de peligro y advertencia, incluyendo las salidas de emergencia y las señales de seguridad.                                                                                     |

## Sección 2 - Sistemas de redes, generalidades y aplicación

[En esta sección se explicarán los diferentes sistemas de redes contra incendio, presentando las normas generales para cada uno, sus generalidades, los compuestos principales, los componentes de la red y los lugares de aplicación.](Section02/Readme.md)

<div align="center">
  <img src="https://github.com/Andrealvch/C.RCI/blob/6b76ba834e285528a118df9cf872eb4359ac6e3e/Section01/.graph/Sistemas%20de%20redes.svg" width="1000px">
</div>

>Se identifican los sistemas de redes 

| Actividad                                                                                                        | Alcance                                                                                                                                                                              |
|:-----------------------------------------------------------------------------------------------------------------|:-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| [Norma para la Instalación de Sistemas de Rociadores - NFPA 13](Section02/NFPA13)                                                    | Se especifican las características y generalidades principales a tener en cuenta para el diseño de la red con agua, sus componentes, usos y aplicaciones                             |
| [Estándar para Espumas Contra Incendios - NFPA 11](Section02/NFPA11)                                                                 | Se especifican las características y generalidades principales a tener en cuenta para el diseño de la red con espumas, sus componentes, usos y aplicaciones                          |
| [Norma sobre Sistemas Extintores de Dióxido de Carbono - NFPA 12](Section02/NFPA12)                                                  | Se especifican las características y generalidades principales a tener en cuenta para el diseño de la red con extintores de dióxido de carbono, sus componentes, usos y aplicaciones |
| [Norma para Sistemas Fijos Aspersores de Agua para Protección Contra Incendios - NFPA 15](Section02/NFPA15)                          | Se especifican las características y generalidades principales a tener en cuenta para el diseño de sistemas fijos aspersores de agua, sus componentes, usos y aplicaciones           |
| [Norma para la Instalación de Sistemas de Rociadores de Agua - Espuma y Pulverizadores de Agua - Espuma - NFPA 16](Section02/NFPA16) | Se especifican las características y generalidades principales a tener en cuenta para el diseño de sistema agua-espuma, sus componentes, usos y aplicaciones                         |
| [Norma para Sistemas Extintores con Productos Químicos Húmedos - NFPA 17A](Section02/NFPA17A)                                         | Se especifican las características y generalidades principales a tener en cuenta para el diseño de sistema con productos químicos húmedos, sus componentes, usos y aplicaciones      |
| [Norma sobre Sistemas de Protección contra Incendios de Agua Nebulizada - NFPA 750](Section02/NFPA750)                                | Se especifican las características y generalidades principales a tener en cuenta para el diseño de sistema con agua nebulizada, sus componentes, usos y aplicaciones                 |
| [Estándar sobre Sistemas de Extinción mediante Agentes Limpios - NFPA 2001](Section02/NFPA2001)                                        | Se especifican las características y generalidades principales a tener en cuenta para el diseño de sistema con agentes limpios, sus componentes, usos y aplicaciones                 |


## Sección 3 - Diseño de las redes

Se presenta el diagrama con el paso a paso del diseño para cada tipo de red con base en su normativa, siempre y cuando este descrita. Para algunos sistemas, el diseño depende del fabricante, tal como lo mencionan las normas vistas en el módulo anterior.

| Actividad         | Alcance                                                                                                                                                                                                                                   |
|:------------------|:------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| [NFPA 13](Section03/NFPA13)           | Para diseñar el sistema de red contra incendio de agua, se utiliza la norma NFPA 13 en el que muestra el paso a paso con las condiciones iniciales para luego realizar el cálculo hidráulico, trazado de la red en planta y de destalles. |
| [NFPA 2001](Section03/NFPA2001)         |  Para diseñar el sistema de red contra incendio mediante agentes limpios, se utiliza la norma NFPA 2001 en el que muestra el paso a paso con las condiciones iniciales, para luego realizar el cálculo hidráulico, trazado de la red en planta y de detalles.                                                                                                                                                                                                                                         |

## Sección 4 - Elementos y sistemas complementarios

| Actividad         | Alcance                                                                                                                                                                                                                                   |
|:------------------|:------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|[Rociadores](Section04/Rociadores)| Descripción de cada tipo de Rociador|
|[Código  Nacional de alarmas de incendio y señalización (NFPA 72)](Section04/Automatizacion)| Descripción breve de el sistema de alarma y señalización que complementa los sistemas contra incendio|

## Sección 5 - Ejemplo diseño de sistema de red contra incendio a base de agua 

[Ejemplo diseño (NFPA 13)](Section05/Readme.md)

# Material del curso por módulos. Notas aclaratorias

Tal como se describió anteriormente, se recomienda que el estudiante revise, estudie y analice la información contenida en cada actividad, siguiendo el orden definido en el contenido programático. En el 
caso que el estudiante requiera la certificación del curso, este deberá realizar las actividades y presentarlas al tutor siguiendo las instrucciones del caso, en los tiempos establecidos.

<div align="center"><a href="https://enlace-academico.escuelaing.edu.co/psc/FORMULARIO/EMPLOYEE/SA/c/EC_LOCALIZACION_RE.LC_FRM_ADMEDCO_FL.GBL" target="_blank"><img src="https://github.com/rcfdtools/R.TeachingResearchGuide/blob/main/CaseUse/.icons/IconCEHBannerCertificado.jpg" alt="R.LTWB" width="100%" border="0" /></a></div>

##

<div align="center"><a href="http://www.escuelaing.edu.co" target="_blank"><img src=".icons/Banner1.svg" alt="Support by" width="100%" border="0" /></a><sub><br>Este curso guía ha sido desarrollado con el apoyo de la Escuela Colombiana de Ingeniería - Julio Garavito. Encuentra más contenidos en https://github.com/uescuelaing</sub><br><br></div>
               

 

